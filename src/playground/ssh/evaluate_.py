...
informer_path = f"weights/electric/14/informer/{str(BEST_LR)}-{str(BEST_WD)}-{str(BEST_EL)}-{str(BEST_DL)}-{str(BEST_DIMENSION)}/state/{str(BEST_EPOCH)}"
...
informer_KL_div_list = plotKL_Div_between_A_and_B(informerA, informerB, "kl_divergence.png", "KL divergence for each forecasts (Informer)")
