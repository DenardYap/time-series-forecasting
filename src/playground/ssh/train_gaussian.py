...
column_to_exclude = 'dt'  # Change this to the column you want to keep

# Define the standard deviation and mean for Gaussian noise
sigma = 1.0
mu = 0.0

# Select the columns to which you want to add noise (excluding the one to keep)
columns_to_noise = [col for col in df_train.columns if col != column_to_exclude]

# Generate Gaussian noise for the selected columns
noise = np.random.normal(mu, sigma, df_train[columns_to_noise].shape)
df_train[columns_to_noise] += noise
noise = np.random.normal(mu, sigma, df_val[columns_to_noise].shape)
df_val[columns_to_noise] += noise
...
INFORMER_PATH = f"weights/electric/{str(prediction_length)}/separate/informer/gaussian/{str(lr)}-{str(weight_decay)}-{str(el)}-{str(dl)}-{str(d)}/"
...
with open(f"best_results_informer_{str(prediction_length)}_gaus.txt", "a") as f:
